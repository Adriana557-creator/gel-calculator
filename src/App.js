...
export default function App() {
  ...
  const pageLabels = { gel: "Gel Mix Calculator", etbr: "EtBr Calculator", template: "Template Selector", dilution: "Sample Dilution", dye: "Loading Dye", buffer: "Buffer Prep", runtime: "Run Time Calculator", protocols: "Protocols" };
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(20);
  pdf.text(pageLabels[page] || page, 20, 20);
  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(11);
  pdf.text("Generated by GelCalc", 20, 30);
  pdf.line(20, 34, 190, 34);
  let y = 44;
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(12);
  pdf.text("Experiment Inputs:", 20, y); y += 8;
  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(11);
  const lines = [
    `Gel Volume: ${inputs.gelVolume || "Not set"} mL`,
    `Gel Percentage: ${inputs.gelPercent}%`,
    `Buffer: ${inputs.buffer}`,
    `Number of Samples: ${inputs.numSamples || "Not set"}`,
    `DNA Concentration: ${inputs.dnaConcInput || "Not set"} ng/µL`,
    `Desired DNA per Lane: ${inputs.desiredDna} ng`,
    `Final Volume per Lane: ${inputs.finalVolume} µL`,
    `Loading Dye Stock: ${inputs.dyeStock}x`,
    `Voltage: ${inputs.voltage} V`,
    `Gel Length: ${inputs.gelLength} cm`,
  ];
  lines.forEach(line => { pdf.text(line, 20, y); y += 7; });
  pdf.line(20, y + 2, 190, y + 2); y += 10;
  pdf.setFont("helvetica", "bold");
  pdf.text("Calculated Results:", 20, y); y += 8;
  pdf.setFont("helvetica", "normal");
  if (page === "gel" && inputs.gelVolume) {
    const vol = parseFloat(inputs.gelVolume);
    const agarose = ((parseFloat(inputs.gelPercent) * vol) / 100).toFixed(3);
    const stockVol = inputs.buffer === "TAE" ? (vol/50).toFixed(2) : (vol/10).toFixed(2);
    const waterVol = (vol - parseFloat(stockVol)).toFixed(2);
    pdf.text(`Agarose powder: ${agarose} g`, 20, y); y += 7;
    pdf.text(`Stock buffer: ${stockVol} mL`, 20, y); y += 7;
    pdf.text(`Distilled water: ${waterVol} mL`, 20, y); y += 7;
  }
  if (page === "etbr" && inputs.gelVolume) {
    const etbr = ((0.5 * parseFloat(inputs.gelVolume)) / (parseFloat(inputs.etbrStock) * 1000) * 1000000).toFixed(2);
    pdf.text(`EtBr to add: ${etbr} µL`, 20, y); y += 7;
    pdf.text(`Final concentration: 0.5 µg/mL`, 20, y); y += 7;
  }
  if (page === "dilution" && inputs.dnaConcInput) {
    const dnaVol = (parseFloat(inputs.desiredDna) / parseFloat(inputs.dnaConcInput)).toFixed(2);
    const waterVol = (parseFloat(inputs.finalVolume) - parseFloat(dnaVol)).toFixed(2);
    const dyeVol = ((parseFloat(dnaVol) + parseFloat(waterVol)) / 5).toFixed(2);
    pdf.text(`DNA volume: ${dnaVol} µL`, 20, y); y += 7;
    pdf.text(`Water: ${waterVol} µL`, 20, y); y += 7;
    pdf.text(`Loading dye: ${dyeVol} µL`, 20, y); y += 7;
  }
  pdf.setFontSize(9);
  pdf.setTextColor(150);
  pdf.text("GelCalc • Agarose Gel Electrophoresis Calculator", 20, 285);
